...
function Rec({bookRec, filteredBooks}) {
  ...
  const handleSubmit = (e) => {
    ...
    const checkIfSaved = bookShelves.find(
        ({title}) => title === filteredBooks[bookRec].title);
  ...
  if (bookRec < 0 || filteredBooks.length === 0) {
    return null;
  }
  const {title, author, img, link, description} = filteredBooks[bookRec];
  return (
    <div className='book'>
